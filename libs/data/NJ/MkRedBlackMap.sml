...
(*
 * Adapted from smlnj-lib/redblack-map-fn.sml
 *
...
functor MkRedBlackMap (Key : ORDERED) :> MAP where type key = Key.t =
  struct
    ...
    fun insertWithi clash (MAP(nItems, m), xk, x) = let
	  val nItems' = ref nItems
	  fun ins E = (nItems' := nItems+1; T(R, E, xk, x, E))
            | ins (s as T(color, a, yk, y, b)) = (case Key.compare(xk, yk)
		 of LESS => (case a
		       of T(R, c, zk, z, d) => (case Key.compare(xk, zk)
			     of LESS => (case ins c
				   of T(R, e, wk, w, f) =>
					T(R, T(B,e,wk, w,f), zk, z, T(B,d,yk,y,b))
                		    | c => T(B, T(R,c,zk,z,d), yk, y, b)
				  (* end case *))
			      | EQUAL => T(color, T(R, c, xk, clash (zk, z, x), d), yk, y, b)
			      | GREATER => (case ins d
				   of T(R, e, wk, w, f) =>
			...
			| _ => T(B, ins a, yk, y, b)
		      (* end case *))
		  | EQUAL => T(color, a, xk, clash (yk, y, x), b)
		  | GREATER => (case b
		       of T(R, c, zk, z, d) => (case Key.compare(xk, zk)
			     of LESS => (case ins c
				   of T(R, e, wk, w, f) =>
				  ...
				  (* end case *))
			      | EQUAL => T(color, a, yk, y, T(R, c, xk, clash (zk, z, x), d))
			      | GREATER => (case ins d
				   of T(R, e, wk, w, f) =>
    ...
    fun insertWith f (m, xk, x) = insertWithi (fn (_, y, x) => f (y, x)) (m, xk, x)
    fun insert (m, xk, x) = insertWithi (fn (_, _, x) => x) (m, xk, x)
    fun insertDisjoint (m, xk, x) = insertWithi (fn (yk, _, _) => raise Collision yk) (m, xk, x)

  (* Is a key in the domain of the map? *)
    fun member (MAP(_, t), k) = let
	  fun find' E = false
	    | find' (T(_, a, yk, y, b)) = (case Key.compare(k, yk)
		 of LESS => find' a
		  | EQUAL => true
		  | GREATER => find' b
		(* end case *))
  ...
  (* Look for an item, return NONE if the item doesn't exist *)
    ...
    fun lookupExistent (m, k) = (case lookup (m, k)
	   of SOME x => x
	    | NONE => raise Unknown k
	   (* end case *))

  (* Remove an item, returning new map and value removed.
   * Raises Unknown if not found.
   *)
    local
      datatype 'a zipper
	= TOP
	| LEFT of (color * key * 'a * 'a tree * 'a zipper)
	| RIGHT of (color * 'a tree * key * 'a * 'a zipper)
    in
    fun removeExistent (MAP(nItems, t), k) = let
	  fun zip (TOP, t) = t
	 ...
	 *)
	  ...
	  fun del (E, z) = raise Unknown k
	    | del (T(color, a, yk, y, b), z) = (case Key.compare(k, yk)
    ...
    fun remove (m, k) = removeExistent (m, k) handle Unknown _ => m
    fun removeWith f (m, k) = removeExistent (m, k) handle Unknown k => (f k; m)

  (* return the first item in the map (or NONE if it is empty) *)
    fun choose (MAP(_, t)) = let
	  fun f E = NONE
...
*)
  ...
  (* Return true if and only if the two maps are equal *)
    fun equal eqRng = let
		 ...
		 of ((E, _), (E, _)) => true
		  ...
		  | ((T(_, _, xk, x, _), r1), (T(_, _, yk, y, _), r2)) => (
		      case Key.compare(xk, yk)
		       of EQUAL => eqRng (x, y) andalso cmp (r1, r2)
			| _ => false
		      (* end case *))
		(* end case *))
	  in
...
(* Return true if and only if the second map extends the first *)
    fun submap eqRng = let
	  fun cmp (t1, t2) = (case (next t1, next t2)
		 of ((E, _), (E, _)) => true
		  | ((E, _), _) => true
		  | (_, (E, _)) => false
		  | ((T(_, _, xk, x, _), r1), (T(_, _, yk, y, _), r2)) => (
		      case Key.compare(xk, yk)
		       of LESS => false
			| EQUAL => eqRng (x, y) andalso cmp (r1, r2)
			| GREATER => cmp (t1, r2)
		      (* end case *))
		(* end case *))
	  in
...
(* Return true if and only if map domains are disjoint *)
    local
	  fun disj (t1, t2) = (case (next t1, next t2)
		...
		(* end case *))
    in
	 fun disjoint (MAP(_, m1), MAP(_, m2)) = disj (start m1, start m2)
    end
...
*)
   ...
   *)
    ...
    fun union (m1, m2) = unionWith #2 (m1, m2)
    fun unionDisjoint (m1, m2) = unionWithi (fn (k, _, _) => raise Collision k) (m1, m2)
   ...
   *)
    ...
    fun intersect (m1, m2) = intersectWith #2 (m1, m2)
    ...
    local
	  fun diff (t1, t2, n, result) = (case (next t1, next t2)
		 of ((E, _), _) => (n, result)
		  ...
		  | ((T(_, _, xk, x, _), r1), (T(_, _, yk, y, _), r2)) => (
		      case Key.compare(xk, yk)
		       of LESS => diff (r1, t2, n+1, addItem(xk, x, result))
			| EQUAL => diff (r1, r2, n, result)
			| GREATER => diff (t1, r2, n, result)
		      (* end case *))
		(* end case *))
    in
	fun difference (m1, m2) = wrap diff (m1, m2)
    end
   ...
   *)
    ...
    fun partition pred (MAP(_, t)) = let
	  fun walk (E, n1, result1, n2, result2) = (n1, result1, n2, result2)
	    | walk (T(_, a, xk, x, b), n1, result1, n2, result2) = let
		val (n1, result1, n2, result2) = walk(a, n1, result1, n2, result2)
		in
		  if (pred x)
		    then walk(b, n1+1, addItem(xk, x, result1), n2, result2)
		    else walk(b, n1, result1, n2+1, addItem(xk, x, result2))
		end
	  val (n1, result1, n2, result2) = walk (t, 0, ZERO, 0, ZERO)
	  in
	    (MAP(n1, linkAll result1), MAP(n2, linkAll result2))
	  end
    fun partitioni pred (MAP(_, t)) = let
	  fun walk (E, n1, result1, n2, result2) = (n1, result1, n2, result2)
	    | walk (T(_, a, xk, x, b), n1, result1, n2, result2) = let
		val (n1, result1, n2, result2) = walk(a, n1, result1, n2, result2)
		in
    ...
    fun exists pred = let
	  fun test E = false
	    | test (T(_, a, _, x, b)) = test a orelse pred x orelse test b
	  in
	    fn (MAP(_, t)) => test t
	  end
    fun existsi pred = let
	  fun test E = false
	    | test (T(_, a, xk, x, b)) = test a orelse pred (xk, x) orelse test b
	  in
	    fn (MAP(_, t)) => test t
	  end
    ...
    fun all pred = let
	  fun test E = true
	    | test (T(_, a, _, x, b)) = test a andalso pred x andalso test b
	  in
	    fn (MAP(_, t)) => test t
	  end
    fun alli pred = let
	  fun test E = true
	    | test (T(_, a, xk, x, b)) = test a andalso pred (xk, x) andalso test b
	  in
	    fn (MAP(_, t)) => test t
	  end
    ...
    fun findi pred = let
	  fun test E = NONE
	    | test (T(_, a, xk, x, b)) = (case test a
		 of NONE => if pred (xk, x) then SOME (xk, x) else test b
		  | someItem => someItem
		(* end case *))
	  in
	    fn (MAP(_, t)) => test t
   ...
   *)
    ...
    fun fromList l = List.foldl (fn ((xk, x), m) => insertDisjoint(m, xk, x)) empty l
    fun toList m = foldri (fn (xk, x, l) => (xk, x)::l) nil m

    fun fromVector v = Vector.foldl (fn ((xk, x), m) => insertDisjoint(m, xk, x)) empty v
